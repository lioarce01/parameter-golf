...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7) -> Tensor:
    # Newton-Schulz orthogonalization for 2D gradient matrices.
...
# -----------------------------
# Validation uses BPB (bits-per-byte) instead of loss, enabling tokenizer-agnostic comparison.
# Note: Submissions editing the tokenizer are examined more carefully.
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        # Keep tied embedding in fp16 (int8 errors compound through both input and output paths)
        if "tok_emb" in name:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_delta=None, v_delta=None) -> Tensor:
        ...
        if self.num_kv_heads != self.num_heads:
            # GQA: expand KV heads (compatible with PyTorch < 2.5)
...
def eval_val_sliding(
    ...
    is_boundary_token_lut: Tensor, seq_len: int, stride: int, eval_batch_seqs: int = 256,
) -> tuple[float, float]:
    ...
    while p + seq_len <= total:
        windows.append((p, 0 if p == 0 else (seq_len - stride)))
        p += stride
    n = len(windows)
    ...
    with torch.inference_mode():
        for i in range(0, len(my_windows), eval_batch_seqs):
            ...
            for b in range(bs):
                ...
                tb += (has_leading_space_lut[scored_targets] & ~is_boundary_token_lut[x[b, s : s + ns]]).to(torch.int16)
...
# -----------------------------
# Per-document LoRA adapters at eval time; each document is independent.
...
def main() -> None:
    ...
    compiled_model = base_model if no_compile else torch.compile(base_model, dynamic=False, fullgraph=True)
